...
全局`auto`在低级原生CLS的ghost_concat配置中解析为新模式；旧E24前瞻配置仍解析为front，新模式明确拒绝旧E24源上下文快照，避免缓存语义错配。复现旧前置方案可给`ghost_concat_job.py`添加`--panorama-context-mode front`，或在`run.py`末尾覆盖`MODEL.RAENWM.panorama_context_mode front`。

旧front检查点允许在不恢复优化器状态的情况下初始化新模式；两种上下文之间的断点续训会被元数据检查拒绝。新检查点保存全景模式、格式、编码/预测批量和缓存设置。新视角编码明确关闭外层混合精度，保持已验证的FP32冻结DINO输入路径，且不调用导航可训练CLS映射。

训练机主工作区验证：87项相关测试通过；双环境真实默认链路完成3次有效调用、19个目标查询，其中10个选择倒序参考端点，目标位置/绝对朝向不变且相对转角均为0；两步联合训练退出0、损失有限、融合梯度范数0.087，479个策略张量更新、CLS映射更新、已保存冻结视觉参数未变。训练日志和模型记录`world_exact_select`。产物位于训练机`data/logs/panorama_default_20260909/`，未启动长训练或完整导航性能测评。
