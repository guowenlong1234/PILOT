...
后续说明（2026-09-05）：本文保留 8 月 9 日零 SFT 与旧视觉接口的历史排查语境。当前工程已取得 DINO 无世界模型 SFT 基线 SR 63.7303%；RGB 注入、低级上下文梯度修复及两台远端最新实验的调查见 [RGB 注入导航效果排查与下一轮实验建议](rgb-injection-navigation-audit-20260905.md)，不要把本文当时的“下一步优先级”直接用于当前实验。
