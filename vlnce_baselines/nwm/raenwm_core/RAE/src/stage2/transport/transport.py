...
class Transport:
    ...
    def __init__(
        self,
        *,
        model_type,             #模型到底学着输出什么，默认是 velocity，也就是让模型预测“往目标走的方向/速度”
        path_type,              ##从随机噪声走到真实样本，中间走哪种路径默认线性
        loss_type,              #loss 的加权方式
        time_dist_type,         ##训练时怎么抽时间 t默认是 uniform，就是均匀采样
        time_dist_shift,        #抽完 t 以后，再把它往某个方向拉一拉
        train_eps,              #
        sample_eps,             #别采到特别贴边的 0 和 1，避免数值不稳定
    ):
        path_options = {                    #“路径类型 -> 具体实现类”的对照表
            PathType.LINEAR: path.ICPlan,   #线性路径。最直观，x_t 大致就是“目标 * (1-t) + 噪声 * t”，默认值
    ...
    def sample(self, x1):
        ...
        x0 = th.randn_like(x1)  #生成一份随机噪声

        dist_options = self.time_dist_type.split("_")   #控制采样分布，比如 uniform 表示均匀采样
        t0, t1 = self.check_interval(self.train_eps, self.sample_eps)   #会给出一个安全范围 [t0, t1]
        if dist_options[0] == "uniform":        #如果是均匀采样
            t = th.rand((x1.shape[0],)) * (t1 - t0) + t0    #就对 batch 里的每个样本随机抽一个 t
            ...
            raise NotImplementedError(f"Unknown time distribution type {self.time_dist_type}")  #未知的采样方法的报错

        t = t.to(x1)        #放置到GPU上

        #sqrt_size_ratio = 1 / self.time_dist_shift # already sqrted
        t = self.time_dist_shift * t / (1 + (self.time_dist_shift - 1) * t)     #对t进行shift变化。
        return t, x0, x1    #时间点 t、噪声起点 x0、真实终点 x1
    ...
    def training_losses(
        ...
        t, x0, x1 = self.sample(x1)     #返回的是时间点 t、噪声起点 x0、真实终点 x1，t是根据采样方法生成出来的。
        t, xt, ut = self.path_sampler.plan(t, x0, x1)   #计算按照t进行均匀混合之后的xt和ut
        model_out = model(xt, t, **model_kwargs)        #模型前向计算
        terms = {}
        if isinstance(model_out, tuple):    #兼容不同模式的返回格式，都保存到pred中
            pred = model_out[0]
            terms['pred'] = pred
            if len(model_out) >= 2:
                terms['intermediate_features'] = model_out[1]
            if len(model_out) >= 3:
                terms['projected_features'] = model_out[2]
        else:
            pred = model_out
            terms['pred'] = pred
        B, *_, C = xt.shape
        assert pred.size() == (B, *xt.size()[1:-1], C)
        if self.model_type == ModelType.VELOCITY:   #如果模型学习的是速度场的话
            base = mean_flat(((pred - ut) ** 2))    #逐个元素平方差误差，MSE
            terms['transport_loss'] = base
            terms['loss'] = base            #在VELOCITY模式下，只有MSE误差项，没有其他项目
            ...
            if self.model_type == ModelType.NOISE:
                terms['loss'] = mean_flat(weight * ((pred - x0) ** 2))
            else:
                terms['loss'] = mean_flat(weight * ((pred * sigma_t + x0) ** 2))
