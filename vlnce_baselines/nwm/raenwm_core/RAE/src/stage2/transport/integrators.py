...
class ode:
    ...
    def sample_final(self, x, model, **model_kwargs) -> th.Tensor:
        """Return only the final fixed-grid Euler state.

        The generic torchdiffeq path stores every state and evaluates Python
        branches on CUDA time scalars.  Current navigation inference consumes
        only the final state, so reproduce Euler's update order directly.
        """
        if self.sampler_type != "euler":
            return self.sample(x, model, **model_kwargs)[-1]
        if isinstance(x, tuple):
            raise TypeError("final-only Euler currently supports tensor states only")

        device = x.device
        time_grid = self.t.to(device)
        time_batch_base = th.ones(x.size(0), device=device)
        state = x
        for t_curr, t_next in zip(time_grid[:-1], time_grid[1:]):
            time_batch = time_batch_base * t_curr
            model_output = self.drift(
                state, time_batch, model, **model_kwargs
            )
            state = state + (t_next - t_curr) * model_output
        return state
