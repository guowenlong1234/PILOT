...
class ICPlan:
    ...
    def compute_alpha_t(self, t):   #与compute_sigma_t配合，控制的是中间的点怎么混合出来，现在是线性插值，所以就是简单的1 - t, -1
    ...
    def compute_drift(self, x, t):
        ...
        t = expand_t_like_x(t, x)   #只是把 t 的形状改成能和 x 对齐
        alpha_ratio = self.compute_d_alpha_alpha_ratio_t(t)     #-1 / (1 - t)，目标那部分权重，现在变化得有多快，而且相对于它当前还剩多少
        sigma_t, d_sigma_t = self.compute_sigma_t(t)    #sigma_t = t，d_sigma_t = 1，当前噪声权重是多少，噪声权重变化速度是多少
        drift = alpha_ratio * x
    ...
    def compute_mu_t(self, t, x0, x1):
        ...
        t = expand_t_like_x(t, x1)              #将t扩充成和x一样的形状
        alpha_t, _ = self.compute_alpha_t(t)
        sigma_t, _ = self.compute_sigma_t(t)
        return alpha_t * x1 + sigma_t * x0      #将x1和xt进行均匀混合
    ...
    def compute_ut(self, t, x0, x1, xt):
        ...
        t = expand_t_like_x(t, x1)              #将t进行广播
        _, d_alpha_t = self.compute_alpha_t(t)  #
        _, d_sigma_t = self.compute_sigma_t(t)
        return d_alpha_t * x1 + d_sigma_t * x0  #计算速度方向

    def plan(self, t, x0, x1):
        xt = self.compute_xt(t, x0, x1)     #计算添加噪音之后的
        ut = self.compute_ut(t, x0, x1, xt) #计算方向场
        return t, xt, ut
