...
2026-09-21主工作区更新：`feature/e24-joint-sft`通过`0903fcc`合入二阶段`ffff1ef`，现在同时包含RxR优化主线与stage2采集、训练、在线评价。论文与绘图目录已取消跟踪并本地忽略，269个文件哈希核验不变。测评机专用容器/环境对独立合并代码执行CPU回归，165通过、1跳过。远端运行目录未更新；详见`docs/main-stage2-merge-20260921.md`。以下分支分离及工作区盘点记录属于此前历史。
...
PILOT 基于 ETP-R1 开发，让智能体在连续三维环境里根据自然语言指令导航到目标位置。当前实现重点为冻结世界模型支持的候选持久表示增强，以及离线训练、在线部署的二阶段评分修正；同时保留基座预训练、SFT、GRPO及R2R/RxR评测入口。根目录README已于2026-09-21按当前代码重写，说明实现边界、环境、资产与实验文档，不再沿用原版ETP-R1官方仓库及成绩声明。
...
2026-09-21，准备9200未来特征内容消融：只用测评机重采train/dev，full/none两组相同结构/初始权重/有效mask/数据顺序与6000步预算。新增无损有效槽存储以满足单机容量。首轮8环境短采因主机OOM退出，改为4环境/编译并行数1后已启动新短测及验收后自动接力的完整流水线（源码62577b2）；当前尚无正式比较结果。协议与位置见 `docs/stage2-9200-future-ablation-20260921.md`。

2026-09-21：9200成对导航评测已全部完成并通过审计，两组各1839路线。直接复用6400训练的4750头×1.5后，SR64.2197%→64.0566%（−0.1631个百分点）、SPL54.6945%→54.6144%（−0.0801），净少3条成功（救回38/丢失41），nDTW下降0.6780个百分点，耗时57.41→97.69分钟（+70.17%）。未发现部署该现成头的收益；不等同于在9200上重训后的结论。详见`docs/stage2-online-9200-navigation-20260920.md`及本地`data/logs/stage2_online_9200_report_20260920/`。下面的运行中条目为历史记录，当前9200队列已结束。

2026-09-20用户新增9200对照：部署基座改为9200，仍复用6400数据训练的4750评分头×1.5；新增显式`6400_to_9200`合同，原6400数据和权重校验不放宽。9200已交付并校验，37项相关检查通过；4次同配置纯基线独立校准后，126步零倍率动作和16路线指标一致，分数差在实测0.00083685范围内。最佳短测已退出0（126决策/4次动作改变）。队列`data/logs/stage2_online_9200_20260920/navigation_v1`已开始完整9200基线，随后自动跑9200＋E24，每组1839路线；最新监督记录`resume1.json`，正式运行版本b6ea0ab。见`docs/stage2-online-9200-navigation-20260920.md`。

基线解释补充：本轮62.86%与历史63.62%都是同SHA的持久6400权重；历史在训练机、未冻结加载开关，本轮在测评机且冻结，尚未单因素定位下降原因。9200历史SR64.06%更高；6400是按本组SPL/SR+SPL选定，不是全项目SR冠军。本轮E24到63.62%不能称超过历史最佳。详见`docs/stage2-baseline-identity-review-20260920.md`。

2026-09-20在线对比已完成并通过审计：V2同版本两组各1839条R2R路线，SR62.8603%→63.6215%（+0.7613个百分点）、SPL54.8048%→55.3100%（+0.5052），净增加14条成功（救回46/丢失32）；耗时55.61→92.76分钟（+66.8%）。SR/SPL按11场景配对95%区间跨0，暂不能确认稳定收益。完整报告见`docs/stage2-online-navigation-20260920.md`和本地`data/logs/stage2_online_report_20260920/`。后面的运行/失败/恢复条目均为历史记录，当前无待完成的在线队列。

在线V2最新门控：同为c259182的两次纯基线分数自差最大0.003911，但动作/指标相同。按原计划使用该独立实测浮点容差，零倍率差0.001038且113步动作/16路线指标完全一致，已通过。1c0fb20仅更新验证规则和报告，最新托管入口为`navigation_v2_geometry_resume1.json`，三项短测均已通过，当前V2正在运行新的完整基线，随后自动评测最佳头；两组均1839路线，最终指标待完成。

当前有效在线队列：`data/logs/stage2_online_20260920/navigation_v2_geometry/pipeline.json`（c259182）。V1在358路线后失败；后续短程与历史基线分数复现门控未通过，未复用旧组做最终比较。V2将先做同版本零倍率门控，再成对重跑两组1839路线。旧结果和失败日志均保留，最终在线指标仍待完成。

2026-09-20在线评测恢复：完整基线SR62.8603%、SPL54.8048%；E24首次在358路线后因地图方向反正弦越界产生NaN而停止。c259182增加边界保护，44项目标环境检查通过；`geometry_recovery_launch.json`托管短程基线一致性检查→全量E24从头重跑，原失败记录保留，尚无完整E24指标。详见`docs/stage2-online-navigation-20260920.md`。

2026-09-20：按用户授权接入离线最佳4750评分头×1.5（±1裁剪），开始R2R在线导航对照。新增`STAGE2_ONLINE`独立开关与`stage2_online.py`，复用采集器无教师的`predict_step`；只在eval启用、参数冻结、停止隔离。测评机继续使用本独立工作区/容器/环境，独立GPU执行流消除了零倍率数值扰动；34行重放和16路线/113步分数、动作、指标逐位一致性检查通过。完整1839路线基线→最佳E24已于9月20日09:39在测评机串行启动，运行根`data/logs/stage2_online_20260920/navigation_v1`，计算源码a14b8e1。状态见`docs/stage2-online-navigation-20260920.md`。
...
2026-09-21，完成9200迁移对照的结果、冻结、路线覆盖和统计区间审计；两组退出0，报告脚本退出0，本地两份报告SHA与远端一致。

2026-09-20，最终核验V2在线成对评测、场景配对区间和冻结证明；两组完整1839路线退出0，报告脚本退出0，本地报告与远端SHA一致。完成本次最佳E24接入与导航指标验证。
